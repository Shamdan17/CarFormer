...
class GPT2Config(PretrainedConfig):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        for attr in ["action", "goal", "reward", "state"]:
            total_width = 0
            for key in kwargs.get("training", {}).get(attr, {}).keys():
                if verbose:
                    print(attr)
                    print(key)
                    print(kwargs["training"][attr])
                    print(kwargs["training"][attr][key])
                width = kwargs["training"][attr][key].get("width", 1)
                total_width += width

            if verbose:
                print("Width of {} is {}".format(attr, total_width))
            setattr(self, f"{attr}_size", total_width)
