...
class LeaderboardEvaluator(object):
    ...
    def _register_statistics(
        self, config, route_date_string, checkpoint, entry_status, crash_message=""
    ):
        """
    ...
    def _load_and_run_scenario(self, args, config):
        ...
        try:
            ...
            if int(os.environ["DATAGEN"]) == 1:
                self.agent_instance = getattr(self.module_agent, agent_class_name)(
                    args.agent_config, config.index, args
                )  # CHANGED
            else:
                self.agent_instance = getattr(self.module_agent, agent_class_name)(
                    args.agent_config, route_date_string, args
                )  # CHANGED
            ...
            result = self._register_statistics(
                config, route_date_string, args.checkpoint, entry_status, crash_message
            )
        ...
        try:
            old_seed = args.trafficManagerSeed
            if int(os.environ["DATAGEN"]) == 1:
                offset = os.environ.get("SEED_OFFSET", 0)
                args.trafficManagerSeed = (
                    config.index + args.trafficManagerSeed + int(offset)
                )
            self._load_and_wait_for_world(args, config.town, config.ego_vehicles)
            args.trafficManagerSeed = old_seed
