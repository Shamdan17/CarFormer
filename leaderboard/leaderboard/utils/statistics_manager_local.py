    ...
    for location in trajectory:
        if previous_location:
            dist = math.sqrt(
                ...
                + (location.y - previous_location.y)
                * (location.y - previous_location.y)
                + (location.z - previous_location.z)
                * (location.z - previous_location.z)
            )
            route_length += dist
...
class StatisticsManager(object):
    ...
    def compute_route_statistics(
        ...
        if not self._registry_route_records or index >= len(
            self._registry_route_records
        ):
        ...
        if score_route > 50.0:
            if hasattr(config.agent, "complete_callback"):
                config.agent.complete_callback()
    ...
    def compute_global_statistics(self, total_routes):
        ...
        if self._registry_route_records:
            for route_record in self._registry_route_records:
                global_record.scores["score_route"] += route_record.scores[
                    "score_route"
                ]
                global_record.scores["score_penalty"] += route_record.scores[
                    "score_penalty"
                ]
                global_record.scores["score_composed"] += route_record.scores[
                    "score_composed"
                ]
                ...
                for key in global_record.infractions.keys():
                    ...
                    if isinstance(global_record.infractions[key], list):
                        global_record.infractions[key] = (
                            len(route_record.infractions[key]) / route_length_kms
                        )
                    else:
                        global_record.infractions[key] += (
                            len(route_record.infractions[key]) / route_length_kms
